...
public class PLSEventsAroundAPlacemarkDB extends BufferAnalyzer {	
	...
	PLSEventsAroundAPlacemarkDB(List<Placemark> ps, double[] radii) {
	...
	*/
	...
	public static void process(Placemark p) throws Exception {
		...
		ps.add(p);
		PLSEventsAroundAPlacemarkDB ba = new PLSEventsAroundAPlacemarkDB(ps, new double[]{p.getRadius()});
		PLSParser.parse(ba);
		ba.finish();
	}
	
	public static void process(List<Placemark> p, double[] r) throws Exception {
		PLSEventsAroundAPlacemarkDB ba = new PLSEventsAroundAPlacemarkDB(p, r);
		PLSParser.parse(ba);
		ba.finish();
	}
